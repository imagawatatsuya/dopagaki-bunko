...
- `追加` で青空文庫の同梱カタログ検索、公開 TXT URL、TXT 貼り付け、手元の ZIP/TXT 取り込み、または同一Wi-Fi上のPC取り込みを使う
- `設定` で `最新状態に更新`、JSON バックアップ、JSON 復元、読書の続き読み込み設定を行う
- エクスポートJSONから、作品ごとの統合TXTをZIPで書き出せる
...
- 検索結果から `図書カード` を開き、公開されている `.txt` URL をそのまま `TXT 公開URL` に入れるか、取得した本文を `TXT を貼り付ける` に貼って取り込める
- 手元ファイルでは `.zip` 内の単一 `.txt`、または単体 `.txt` を Shift_JIS 優先で読んで断片化する
- `TXT 公開URL` は設定に保存され、次回の取り込み時にも残る
- PC中継から受け取った候補は `importReceipts` としてIndexedDBへ保存する
- 作品保存と完了記録は同じIndexedDBトランザクションで確定し、完了・失敗した配送IDは再送や再起動後も取り込み候補へ戻さない
- 中継ページへの受信確認は、取り込み候補をIndexedDBへ保存してプレビューを生成した後に返す。一時的なDBエラーでは失敗確定せず、候補を再試行できる状態に保つ
- 同一Wi-Fi上のPCで `aozora-converter-for-dopagaki` を動かし、`PCのURL` から作品一覧または個別作品を中継して取り込める
...
`aozora-converter-for-dopagaki` を同一Wi-Fi上で動かしている場合は、`PCからプレビューを開く` が主導線です。PC上の候補一覧と個別中継ページが本文を `dopagaki-bunko` へ返すので、貼り付けなしでプレビューまで進みます。詳細は [docs/converter-integration.md](/C:/Users/xylitol/github_p/dopagaki-bunko/docs/converter-integration.md) を参照してください。
...
- `JSONからTXT ZIPを書き出す` で、バックアップJSONから作品ごとの統合TXTをまとめて保存する
- `Google Driveに保存` で、対応ブラウザの共有画面からTXT ZIPの保存先にGoogle Driveを選べる
...
- [docs/converter-integration.md](/C:/Users/xylitol/github_p/dopagaki-bunko/docs/converter-integration.md)
