...
`aozora-converter-for-dopagaki` の GitHub Pages QR 実験モードでは、まず PC 上の中継ページが `latest.txt` を読み、その本文を `window.name` 経由で `dopagaki-bunko` に渡します。この導線では貼り付けなしでそのままプレビュー画面まで進みます。
