...
6. LAN の `PCの最新作を読む` や `remoteImportUrl=http://<PCのIP>:8765/latest.txt` は、まずそのまま試し、失敗したら画面の確認項目に従って切り分ける。
...
`aozora-converter-for-dopagaki` を同一Wi-Fi上で動かしている場合は、`PCからプレビューを開く` が主導線です。PC 上の中継ページが `latest.txt` を読み、その本文を `dopagaki-bunko` へ返すので、貼り付けなしでそのままプレビュー画面まで進みます。
